    ...
    def check_merge(self, feature_list):
        ...
        for fc in feature_list:
            ...
            if fc is not None:
                ...
                winter_pre_name=fr"{self.e_winter_pre}_{feature_list[fc]}"

                winter_post="Year >= 1998 And Cat ='Winter'"
                winter_post_name=fr"{self.e_winter_post}_{feature_list[fc]}"

                movement_pre="Year < 1998 And Cat ='Movement'" 
                movement_pre_name=fr"{self.e_movement_pre}_{feature_list[fc]}"

                movement_post="Year < 1998 And Cat >='Movement'" 
                movement_post_name=fr"{self.e_movement_post}_{feature_list[fc]}"          
                
            

                if before_1998_count >0:
                    logging.info('Looking for features before 1998 to append')
                    arcpy.AddMessage('Looking for features before 1998 to append')
                    ...
                    if final_count >0:                
                        ...
                        self.tracker.append_tracker(winter_pre_name,int(final_count), str(feature_list[fc]), str(self.usr_nm))
                        logging.info('winter pre 1998 appended')
                        arcpy.AddMessage('Winter pre 1998 appended....')
                        arcpy.SelectLayerByAttribute_management(key, "CLEAR_SELECTION")
                    else:
                        logging.info(f"No new features for {winter_pre_name}")
                        arcpy.AddMessage(f"No new features for {winter_pre_name}")
                        arcpy.SelectLayerByAttribute_management(key, "CLEAR_SELECTION")  
                    ...
                    if final_count >0:                     
                        ...
                        fieldMappings.removeAll()
                        self.tracker.append_tracker(winter_post_name,int(final_count), str(feature_list[fc]), str(self.usr_nm))
                        logging.info('winter post 1998 appended')
                        arcpy.AddMessage('winter post 1998 appended...')
                        arcpy.SelectLayerByAttribute_management(key, "CLEAR_SELECTION")
                    else:
                        logging.info(f"No new features for {winter_post_name}")
                        arcpy.AddMessage(f"No new features for {winter_post_name}")
                        arcpy.SelectLayerByAttribute_management(key, "CLEAR_SELECTION")      
                    ...
                    if final_count >0: 
                        ...
                        fieldMappings.removeAll()
                        self.tracker.append_tracker(movement_pre_name,int(final_count), str(feature_list[fc]), str(self.usr_nm))
                        logging.info('Movement pre 1998 appended')
                        arcpy.AddMessage('Movement pre 1998 appended....')
                        arcpy.SelectLayerByAttribute_management(key, "CLEAR_SELECTION")
                    else:
                        logging.info(f"No new features for {movement_pre_name}")
                        arcpy.AddMessage(f"No new features for {movement_pre_name}")
                        arcpy.SelectLayerByAttribute_management(key, "CLEAR_SELECTION")  
                    ...
                    if final_count >0: 
                        ...
                        fieldMappings.removeAll()
                        self.tracker.append_tracker(movement_post_name,int(final_count), str(feature_list[fc]), str(self.usr_nm))
                        logging.info('Movement post 1998 appended')
                        arcpy.AddMessage('Movement post 1998 appended....')
                        arcpy.SelectLayerByAttribute_management(key, "CLEAR_SELECTION")
                    else:
                        logging.info(f"No new features for {movement_post_name}")
                        arcpy.AddMessage(f"No new features for {movement_post_name}")
                        arcpy.SelectLayerByAttribute_management(key, "CLEAR_SELECTION")  
                                                    
                
                else:
                    logging.info('Looking for features after 1998 to append')
                    arcpy.AddMessage('Looking for features after 1998 to append....')
                    ...
                    if final_count >0:
                        ...
                        arcpy.management.Append(key, winter_post_name, schema_type= "NO_TEST", field_mapping=fieldMappings, expression=winter_post) 
                        fieldMappings.removeAll()
                        self.tracker.append_tracker(str(winter_pre_name),(final_count), str(feature_list[fc]), str(self.usr_nm))      
                        logging.info('post winter features appended')
                        arcpy.AddMessage('Post winter features appended')
                        arcpy.SelectLayerByAttribute_management(key, "CLEAR_SELECTION")
                    else:
                        logging.info(f"No new features for {winter_post_name}")
                        arcpy.AddMessage(f"No new features for {winter_post_name}")
                    ...
                    if final_count >0:                    
                        ...
                        arcpy.AddMessage('Post movement features appended')
                        arcpy.SelectLayerByAttribute_management(key, "CLEAR_SELECTION")
                    else:
                        logging.info(f"No new features for {movement_post_name}")
                        arcpy.AddMessage(f"No new features for {movement_post_name}")
